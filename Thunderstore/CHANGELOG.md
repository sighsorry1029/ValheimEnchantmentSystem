...
| Version | UpdateNotes |
|---|---|
| 1.9.12 | - Updated Expand World Data integration for version 1.68 by using the current `ReadConfigs` reload hook, removing legacy `FromFile` compatibility, and eliminating repeated HarmonyX missing-method warnings |
| 1.9.11 | - Fixed skill EXP scrolls not being consumed with the E interaction on dedicated servers while retaining server-side prefab, distance, and duplicate-consumption validation |
